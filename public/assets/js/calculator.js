(function () {
  function calcT(key, fb) {
    const p = window.CALC_I18N;
    if (p && p[key] != null && p[key] !== '') return p[key];
    return fb !== undefined ? fb : key;
  }
  function withCalcI18n(arr, prefix, fields) {
    fields = fields || ['n', 'd'];
    if (!window.CALC_I18N) return arr;
    return arr.map((row) => {
      const out = Object.assign({}, row);
      fields.forEach((f) => {
        if (row[f] != null) out[f] = calcT(prefix + '.' + row.id + '.' + f, row[f]);
      });
      return out;
    });
  }
  function smartI18n(smart) {
    if (!window.CALC_I18N) return smart;
    const out = {};
    Object.keys(smart).forEach((k) => {
      const sk = k === '' ? 'default' : k;
      const v = smart[k];
      out[k] = {
        title: calcT('calc.smart.' + sk + '.title', v.title),
        text: calcT('calc.smart.' + sk + '.text', v.text),
      };
    });
    return out;
  }

  const TYPE_CATS = withCalcI18n([
    { id: 'all', n: 'Hamısı' },
    { id: 'web', n: 'Veb sayt' },
    { id: 'mobile', n: 'Mobil' },
  ...
  TYPES = withCalcI18n(TYPES, 'calc.types');
  ...
  function stepTitle(n) {
    if (n === 0) return calcT('calc.step.0', 'Layihə növü');
    if (n === 1) return $('scopeTitle')?.textContent || calcT('calc.step.1', 'Layihə həcmi');
    if (n === 2) return $('modPanelTitle')?.textContent || calcT('calc.step.2', 'Modullar');
    if (n === 3) return calcT('calc.step.3', 'Dizayn & müddət');
    return calcT('calc.step.4', 'Pulsuz təklif');
  }
  ...
  function bottomCtaConfig() {
    ...
    const map = [
      { label: calcT('calc.cta.continue', 'Davam et'), disabled: !S.type },
      { label: calcT('calc.cta.modules', 'Modullar'), disabled: false },
      { label: calcT('calc.cta.design', 'Dizayn'), disabled: false },
      { label: calcT('calc.cta.quote', 'Təklif al'), disabled: false },
      { label: calcT('calc.cta.send', 'Göndər'), disabled: false },
    ];
  ...
  function updateMobileShell() {
    ...
    if ($('calcStepSubtitle')) {
      $('calcStepSubtitle').textContent =
        curStep === 0
          ? $('smartText')?.textContent || calcT('calc.step.0.sub', 'Layihə növünü seçin')
  ...
  async function submitForm() {
    ...
    let ok = false;
    try {
      const res = await fetch((window.MIRTECH?.base || '') + '/api.php?action=lead', {
        method: 'POST',
